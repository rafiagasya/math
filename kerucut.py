...
class KerucutCalculator(tk.Frame):
    ...
    def create_widgets(self):
        # Title
        title = tk.Label(self, text="Kalkulator Kerucut", bg="#f0f0f0", fg="#2c3e50",
                        font=("Arial", 18, "bold"))
        title.pack(pady=20)
        ...
        # Garis Pelukis
        tk.Label(form_frame, text="Garis Pelukis (cm):", bg="#f0f0f0", fg="#333",
                font=("Arial", 11)).grid(row=2, column=0, sticky=tk.W, pady=5)
        self.garis_pelukis = tk.Entry(form_frame, font=("Arial", 11), width=30)
        self.garis_pelukis.grid(row=2, column=1, pady=5, padx=10)
    ...
    def calculate(self):
        try:
            ...
            s = float(self.garis_pelukis.get())
            ...
            result_text = f"""HASIL PERHITUNGAN KERUCUT
{'─'*40}
Jari-jari: {r} cm
Tinggi: {t} cm
Garis Pelukis: {s} cm
...
Luas Permukaan: {luas_permukaan:.2f} cm²"""
    ...
    def reset(self):
        ...
        self.garis_pelukis.delete(0, tk.END)
        self.result.delete(1.0, tk.END)
